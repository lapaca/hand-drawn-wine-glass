# 🍷 Hand-Drawn Swirling Wine Glass (超高性能手绘动态红酒杯)
...
一个极简、极速、无任何外部依赖的**手绘铅笔风格**红酒杯动态交互效果。采用创新的**数学物理降维技术**，让精美的手绘动效渲染性能拉满！
...
* **我们的创新解法**：通过**将流体物理降维至纯 CSS 层面**。使用两组超宽的贝塞尔曲线（Bezier Curves）做无缝位移产生波动，利用 GPU 硬件加速直接渲染。
* **缓动回弹数学公式**：通过巧妙地将外设倾角传入，配合自定义贝塞尔曲线 `cubic-bezier(0.175, 0.885, 0.32, 1.275)` 进行运动缓冲，实现了极具**流体惯性与粘滞回弹质感**的动态反馈，**CPU 开销趋近于 0**！
